...
testQuery() {
    set -x
    peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile ${FABRIC_PATH}/test-network/organizations/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem -C mychannel -n ${CC_NAME} --peerAddresses localhost:7051 --tlsRootCertFiles ${FABRIC_PATH}/test-network/organizations/peerOrganizations/org1.example.com/peers/peer0.org1.example.com/tls/ca.crt --peerAddresses localhost:9051 --tlsRootCertFiles ${FABRIC_PATH}/test-network/organizations/peerOrganizations/org2.example.com/peers/peer0.org2.example.com/tls/ca.crt -c '{"function":"initSensor","Args":[]}'
    peer chaincode query -C mychannel -n ${CC_NAME} -c '{"Args":["querySensor", "1"]}'
    res=$?
	set +x
	cat log.txt
	echo "===================== Chaincode commited  on peer0.org${ORG} on channel  ===================== "
